...
export default function Home() {
  ...
  const handleQuickLink = (e: React.MouseEvent<HTMLAnchorElement>, href: string, label: string) => {
    if (!isAuthenticated) {
      e.preventDefault();
      setPasscodeModal({ driveName: label, targetHref: href });
    }
  };
    ...
    >
      ...
      >
        ...
        <div className="mt-5 flex gap-2">
          <Link href="/favorites" onClick={(e) => handleQuickLink(e, "/favorites", "Favorites")} className="flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-white/60 backdrop-blur-sm transition-all hover:border-yellow-400/40 hover:text-yellow-300">
            <svg className="h-3.5 w-3.5" fill="currentColor" viewBox="0 0 24 24">
              <path d="M11.48 3.499a.562.562 0 0 1 1.04 0l2.125 5.111a.563.563 0 0 0 .475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 0 0-.182.557l1.285 5.385a.562.562 0 0 1-.84.61l-4.725-2.885a.562.562 0 0 0-.586 0L6.982 20.54a.562.562 0 0 1-.84-.61l1.285-5.386a.562.562 0 0 0-.182-.557l-4.204-3.602a.562.562 0 0 1 .321-.988l5.518-.442a.563.563 0 0 0 .475-.345L11.48 3.5Z" />
            </svg>
            お気に入り
          </Link>
          <Link href="/search" onClick={(e) => handleQuickLink(e, "/search", "検索")} className="flex items-center gap-1.5 rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-white/60 backdrop-blur-sm transition-all hover:border-cyan-400/40 hover:text-cyan-300">
